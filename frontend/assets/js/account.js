(() => {
  ...
  const consumeOAuthResult = () => {
    if (!location.hash || !location.hash.startsWith("#oauth_")) return;
    ...
    if (oauthToken) {
      ...
      sessionStorage.setItem("tlr_oauth_notice", `Signed in with ${provider === "discord" ? "Discord" : "Google"}.`);
    } else if (oauthError) {
      sessionStorage.setItem("tlr_oauth_error", oauthError);
    }
  };
  ...
  const renderUser = (user) => {
    ...
    const methods = Array.isArray(user.sign_in_methods) && user.sign_in_methods.length
      ? user.sign_in_methods
      : ["email"];
    $("#details-auth-methods").textContent = methods.map((method) => {
    ...
    if (linked) {
      ...
      $("#linked-minecraft-name").textContent = "";
      $("#linked-minecraft-head")?.removeAttribute("src");
      const head = $("#profile-head");
      head?.removeAttribute("src");
      show(head, false);
      show($("#profile-placeholder"), true);
    }
  ...
  const setupAuthentication = () => {
    ...
    const beginOAuth = (provider) => {
      if (!API_BASE) return setResult(result, "The website backend is not connected yet.", true);
      const returnTo = `${location.origin}${location.pathname}`;
      location.assign(`${API_BASE}/api/auth/oauth/${provider}/start?return_to=${encodeURIComponent(returnTo)}`);
    };

    $("#google-signin-button")?.addEventListener("click", () => beginOAuth("google"));
    $("#discord-signin-button")?.addEventListener("click", () => beginOAuth("discord"));

    const oauthError = sessionStorage.getItem("tlr_oauth_error");
    const oauthNotice = sessionStorage.getItem("tlr_oauth_notice");
    if (oauthError) {
      sessionStorage.removeItem("tlr_oauth_error");
      setResult(result, oauthError, true);
    } else if (oauthNotice) {
      sessionStorage.removeItem("tlr_oauth_notice");
      setResult(result, oauthNotice);
    }
  ...
  const setupMinecraftUnlink = () => {
    const modal = $("#minecraft-unlink-modal");
    const result = $("#minecraft-unlink-result");
    ...
    const openModal = () => {
      if (!currentUser?.minecraft_uuid || !currentUser?.minecraft_name) return;
      $("#unlink-minecraft-name").textContent = currentUser.minecraft_name;
      $("#unlink-website-email").textContent = currentUser.email || "—";
      $("#unlink-minecraft-uuid").textContent = `UUID: ${currentUser.minecraft_uuid}`;
      setResult(result, "");
      show(modal, true);
      document.body.classList.add("portal-modal-open");
    };

    $("#open-unlink-minecraft-button")?.addEventListener("click", openModal);
    $("#close-minecraft-unlink-button")?.addEventListener("click", closeModal);
    document.querySelectorAll("[data-close-minecraft-unlink]").forEach((button) => button.addEventListener("click", closeModal));
    document.addEventListener("keydown", (event) => {
      if (event.key === "Escape" && modal && !modal.classList.contains("hidden")) closeModal();
    });

    $("#confirm-unlink-minecraft-button")?.addEventListener("click", async () => {
      if (!currentUser?.minecraft_uuid) return;
      const button = $("#confirm-unlink-minecraft-button");
      const linkedName = currentUser.minecraft_name || "Minecraft account";
      if (button) button.disabled = true;
      setResult(result, "Unlinking your Minecraft account...");
      try {
        const data = await request("/api/account/unlink", { method: "POST" }, true);
        const refreshed = await request("/api/me", {}, true);
        renderUser(refreshed);
        closeModal();
        setResult($("#link-result"), data.message || `${linkedName} was unlinked successfully.`);
      } catch (error) {
