(() => {
  ...
  const setupActions = () => {
    ...
    $$("[data-discord]").forEach((link) => {
      link.href = cfg.DISCORD_INVITE || "https://discord.gg/4Rfx4pwV";
      link.target = "_blank";
      link.rel = "noopener noreferrer";
    });

    $$("[data-preview-action]").forEach((button) => {
      button.addEventListener("click", () => toast(button.dataset.previewAction || "This feature is coming in the next release."));
    });
  };
  ...
  const updateStatus = async () => {
    ...
    const setDemo = () => {
      if (statusText) statusText.textContent = "Development Realm • Demo status";
      if (onlineCount) onlineCount.textContent = "42";
      if (statusOrb) statusOrb.classList.remove("offline");
    };
  ...
  const setupPortalForm = () => {
    ...
    form.addEventListener("submit", (event) => {
      ...
      toast("Account authentication arrives in Version 2. This preview is visual only.");
    });
  };
