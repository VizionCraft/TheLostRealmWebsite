...
async function routeRequest(request, env) {
  ...
  if (method === "POST" && path === "/api/account/unlink") return unlinkAccount(request, env);
...
function isAllowedOrigin(origin, env) {
  ...
  // Always trust this project's production Pages hostname and Cloudflare Pages
  // preview deployments. Preview deployments use hostnames such as
  // abc123.thelostrealmwebsite.pages.dev, so they should not require a new
  // ALLOWED_ORIGINS entry after every frontend deployment.
  try {
    const originUrl = new URL(normalizedOrigin);
    const host = originUrl.hostname.toLowerCase();
    if (
      originUrl.protocol === "https:" &&
      (host === "thelostrealmwebsite.pages.dev" ||
        host.endsWith(".thelostrealmwebsite.pages.dev"))
    ) {
      return true;
    }
  } catch {
...
async function requestData(request, env) {
  ...
  return json({ message: "Your Lost Realm account data card was emailed to you." });
}
...
async function unlinkAccount(request, env) {
  const user = await currentUser(request, env);
  if (!user.minecraft_uuid || !user.minecraft_name) {
    throw new ApiError(409, "No Minecraft account is currently linked to this website account.");
  }

  const now = nowTs();
  const minecraftUuid = user.minecraft_uuid;
  const minecraftName = user.minecraft_name;

  await env.DB.batch([
    env.DB.prepare("UPDATE users SET minecraft_uuid = NULL, minecraft_name = NULL WHERE id = ?1").bind(user.id),
    env.DB.prepare("UPDATE link_codes SET used_at = ?1 WHERE minecraft_uuid = ?2 AND used_at IS NULL").bind(now, minecraftUuid),
  ]);

  return json({
    message: `${minecraftName} was unlinked from this website account. You can reconnect anytime with /link.`,
  });
}
...
async function sendDataExport(env, email, data) {
  const value = (input, fallback = "Not set") => {
    if (input === null || input === undefined || input === "") return fallback;
    return String(input);
  };

  const playtimeMinutes = Number(data.playtime_minutes || 0);
  ...
  const plainRows = [
    ...
    ["minecraft_uuid", value(data.minecraft_uuid)],
    ["minecraft_name", value(data.minecraft_name)],
    ["playtime_minutes", value(data.playtime_minutes, "0")],
    ["quests_completed", value(data.quests_completed, "0")],
    ["achievements", value(data.achievements, "0")],
  ...
  const text = [
    ...
    "MINECRAFT PROFILE",
    "────────────────────────",
    `Minecraft name: ${value(data.minecraft_name)}`,
    `Minecraft UUID: ${value(data.minecraft_uuid)}`,
    "",
    "REALM PROGRESS",
    "────────────────────────",
    `Rank: ${value(data.playtime_rank)}`,
    `Display title: ${value(data.display_title)}`,
    `Playtime: ${playtimeDisplay}`,
    `Quests completed: ${value(data.quests_completed, "0")}`,
    `Achievements: ${value(data.achievements, "0")}`,
    `Friends: ${value(data.friends, "0")}`,
    ...(data.is_admin === true ? ["", "ADMINISTRATOR", "────────────────────────", "is_admin: true"] : []),
    "",
    `Exported: ${value(data.exported_at)}`,
    "",
    "This export was requested from your signed-in Lost Realm account.",
  ].join("\n");

  const row = (label, val, accent = false) => `
    <tr>
      <td style="padding:10px 0;color:#8e8374;font-size:12px;letter-spacing:.8px;text-transform:uppercase;border-bottom:1px solid #2b241c;width:42%;vertical-align:top">${escapeHtml(label)}</td>
      <td style="padding:10px 0;color:${accent ? "#f6dda2" : "#eee6da"};font-size:14px;font-weight:${accent ? "700" : "500"};border-bottom:1px solid #2b241c;word-break:break-word;vertical-align:top">${escapeHtml(value(val))}</td>
    </tr>`;

  const adminRow = data.is_admin === true
    ? row("is_admin", "true", true)
    : "";

  const minecraftBlock = data.minecraft_name
    ? `
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-top:18px;background:#15110e;border:1px solid #3b3024;border-radius:14px">
        <tr>
          <td style="padding:18px">
            <div style="color:#e2b869;font-size:11px;font-weight:700;letter-spacing:2.4px;text-transform:uppercase;margin-bottom:8px">Minecraft Profile</div>
            <div style="font-family:Georgia,serif;color:#f5eee4;font-size:22px;font-weight:700">${escapeHtml(value(data.minecraft_name))}</div>
            <div style="color:#8e8374;font-size:12px;margin-top:7px;word-break:break-all">UUID: ${escapeHtml(value(data.minecraft_uuid))}</div>
          </td>
        </tr>
      </table>`
    : `
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-top:18px;background:#15110e;border:1px solid #3b3024;border-radius:14px">
        <tr><td style="padding:18px">
          <div style="color:#e2b869;font-size:11px;font-weight:700;letter-spacing:2.4px;text-transform:uppercase;margin-bottom:8px">Minecraft Profile</div>
          <div style="color:#8e8374;font-size:14px">No Minecraft account is currently linked.</div>
        </td></tr>
      </table>`;

  const html = `
  <div style="margin:0;padding:0;background:#090706;font-family:Arial,Helvetica,sans-serif;color:#eee6da">
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#090706;padding:28px 12px">
      <tr>
        <td align="center">
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:640px;background:#100d0a;border:1px solid #4e3c27;border-radius:22px;overflow:hidden;box-shadow:0 18px 60px rgba(0,0,0,.42)">
            <tr>
              <td style="padding:32px 34px 27px;background:linear-gradient(135deg,#1d160f 0%,#0e0b08 70%);border-bottom:1px solid #4e3c27">
                <div style="color:#e2b869;font-size:11px;font-weight:800;letter-spacing:3.6px;text-transform:uppercase">THE LOST REALM</div>
                <div style="font-family:Georgia,serif;color:#fff8ed;font-size:32px;font-weight:700;margin-top:8px">Account Data</div>
                <div style="color:#9e9385;font-size:14px;line-height:1.6;margin-top:10px">A private snapshot of the information currently stored for your Lost Realm website account.</div>
              </td>
            </tr>
            <tr>
              <td style="padding:28px 34px 12px">
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-bottom:18px">
                  <tr>
                    <td style="padding:15px 14px;background:#17120e;border:1px solid #3b3024;border-radius:13px;text-align:center;width:33%">
                      <div style="font-family:Georgia,serif;font-size:21px;color:#f6dda2;font-weight:700">${escapeHtml(value(data.playtime_rank, "Traveler"))}</div>
                      <div style="font-size:10px;color:#887d70;letter-spacing:1.8px;text-transform:uppercase;margin-top:6px">Rank</div>
                    </td>
                    <td style="width:10px"></td>
                    <td style="padding:15px 14px;background:#17120e;border:1px solid #3b3024;border-radius:13px;text-align:center;width:33%">
                      <div style="font-family:Georgia,serif;font-size:21px;color:#f6dda2;font-weight:700">${escapeHtml(playtimeDisplay)}</div>
                      <div style="font-size:10px;color:#887d70;letter-spacing:1.8px;text-transform:uppercase;margin-top:6px">Playtime</div>
                    </td>
                    <td style="width:10px"></td>
                    <td style="padding:15px 14px;background:#17120e;border:1px solid #3b3024;border-radius:13px;text-align:center;width:33%">
                      <div style="font-family:Georgia,serif;font-size:21px;color:#f6dda2;font-weight:700">${escapeHtml(value(data.quests_completed, "0"))}</div>
                      <div style="font-size:10px;color:#887d70;letter-spacing:1.8px;text-transform:uppercase;margin-top:6px">Quests</div>
                    </td>
                  </tr>
                </table>
                ...
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-top:18px;background:#15110e;border:1px solid #3b3024;border-radius:14px">
                  <tr>
                    <td style="padding:18px 20px">
                      <div style="color:#e2b869;font-size:11px;font-weight:700;letter-spacing:2.4px;text-transform:uppercase;margin-bottom:5px">Stored Account Fields</div>
                      <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
                        ${row("id", data.id)}
                        ${row("email", data.email, true)}
                        ${row("created_at", data.created_at)}
                        ${row("last_login", data.last_login)}
                        ${row("playtime_minutes", data.playtime_minutes, false)}
                        ${row("quests_completed", data.quests_completed, false)}
                        ${row("achievements", data.achievements, false)}
                ...
                <div style="padding:22px 2px 26px;color:#746b60;font-size:12px;line-height:1.6;text-align:center">
                  This export was requested from your signed-in Lost Realm account.<br>Keep this email private because it contains account information.
                </div>
              </td>
            </tr>
          </table>
